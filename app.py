...
def save_interaction(user_input, response):
    conn = sqlite3.connect('data/your_database.db')
    c = conn.cursor()
    c.execute('INSERT INTO interactions (user_input, response) VALUES (?, ?)', (user_input, response))
    conn.commit()
    conn.close()
...
if st.button("Send"):
    if user_input.strip() != "":
        try:
            # Call DeepAI API to generate response
...
# Option to show past interactions
...
if st.checkbox("Show past interactions"):
    ...
    for i, (inp, resp) in enumerate(rows, 1):
        st.write(f"**User:** {inp}")
        st.write(f"**AI:** {resp}")
        st.write("---")if st.checkbox("Show past interactions"):
    conn = sqlite3.connect('data/your_database.db')
    c = conn.cursor()
    c.execute("SELECT user_input, response FROM interactions ORDER BY id DESC LIMIT 10")
    rows = c.fetchall()
